...
import type { ChatConfig, GenerationConfig } from './config';
...
export class LLMChatPipeline {
        ...
        grammarMatcherInitPromise = new Promise(async (resolve) => {
          ...
          const grammar: xgr.CompiledGrammar =
            curSchemaOrGrammarStr === undefined
              ...
              : genConfig?.response_format?.type === 'json_object'
                ? await this.grammarCompiler!.compileJSONSchema(curSchemaOrGrammarStr)
                : await this.grammarCompiler!.compileGrammar(curSchemaOrGrammarStr);
  ...
  private async sampleTokenFromLogits(logitsOnGPU: tvmjs.Tensor, genConfig?: GenerationConfig) {
    ...
    if (response_format?.type === 'json_object' || response_format?.type === 'grammar') {
    ...
    if (this.logitProcessor !== undefined) {
      ...
      let logitsOnCPUArray: Float32Array = this.logitsOnCPU.toArray() as Float32Array;
  ...
  private getTokenLogprob(sampledToken: number, top_logprobs: number): ChatCompletionTokenLogprob {
    ...
    const logitsOnCPUArray = this.logitsOnCPU.toArray() as Float32Array;

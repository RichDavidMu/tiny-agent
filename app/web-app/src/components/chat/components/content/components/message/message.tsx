...
import type { AgentState } from '@tini-agent/agent-core';
...
export const Message = observer(
  ({ node, isCurrentNode, status }: { node: Node; isCurrentNode: boolean; status: AgentState }) => {
    return (
      <MessageBox className={cn(node.role === 'user' ? 'flex-row-reverse' : 'flex-row')}>
        ...
        >
          ...
          {node.role === 'assistant' && isCurrentNode && (
            <StatusBlock status={status} node={node} />
          )}
        </MessageContent>
      </MessageBox>
    );

...
export class MLCEngine implements MLCEngineInterface {
  ...
  private async reloadInternal(modelId: string, chatOpts?: ChatOptions): Promise<void> {
    ...
    const curModelConfig = {
      ...(await configCache.fetchWithCache(configUrl, 'json', this.reloadController?.signal)),
    ...
    if (modelRecord.model_type === ModelType.embedding) {
      ...
      newPipeline = new LLMChatPipeline(tvm, this.tokenizer, curModelConfig, logitProcessor);
    }
